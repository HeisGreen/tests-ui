...
export async function triggerGoogleSignIn() {
  if (!GOOGLE_CLIENT_ID) {
    ...
    console.error(
      "Please add VITE_GOOGLE_CLIENT_ID to your Frontend/.env file"
    );
    console.error(
  ...
  if (GOOGLE_CLIENT_ID.length > 20) {
    const maskedId =
      ...
      "..." +
      GOOGLE_CLIENT_ID.substring(GOOGLE_CLIENT_ID.length - 10);
    console.log("✅ Google Client ID loaded:", maskedId);
  } else {
    console.warn(
      "⚠️ Google Client ID seems too short. Please verify it's correct."
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!resolved) {
        ...
        reject(
          new Error(
            "Google sign-in timed out or was cancelled. Please try again."
          )
        );
      }
    }, 300000);
